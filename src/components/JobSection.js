...
export default function JobSection({ jobs, handleClick, name, data }) {
  ...
  return (
    <Box sx={{ overflow: "hidden" }}>
      <Box sx={{ m: 1 }}>
        <Typography sx={{ fontSize: 40, fontWeight: 700, font: "inter", font: 'black'}}>
          {name}
        </Typography>
      </Box>
      {loading ? (
            ...
            {jobs.map((row, index) => {
              return (
                <Grid item xs={1} sm={1} md={1} key={index}>
                  <JobCard row={row} handleClick={handleClick} />
                </Grid>
              );
            })}
          </Grid>
